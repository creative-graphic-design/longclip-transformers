...
def load(
    ...
    model = build_model(state_dict or model.state_dict(), load_from_clip=False).to(
        device
    )
...
def load_from_clip(
    ...
    model = build_model(state_dict or model.state_dict(), load_from_clip=True).to(
        device
    )
    ...
    model.positional_embedding = nn.Parameter(
        posisitonal_embedding_new, requires_grad=False
    )
