...
with torch.no_grad():
    ...
    def image2image(
        ...
        **kwargs,
        ...
        pipe._denoising_start = denoising_start
        ...
        timesteps, num_inference_steps = pipe.get_timesteps(
            ...
            strength,
            device,
            denoising_start=pipe.denoising_start if denoising_value_valid else None,
        )
        latent_timestep = timesteps[:1].repeat(batch_size * num_images_per_prompt)

        add_noise = True if pipe.denoising_start is None else False
        ...
        if (
            ...
            and pipe.denoising_start is not None
            and denoising_value_valid(pipe.denoising_end)
            and denoising_value_valid(pipe.denoising_start)
            and pipe.denoising_start >= pipe.denoising_end
        ):
            raise ValueError(
