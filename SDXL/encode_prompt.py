...
def kps(model):
    ...
    for i in range(length - 1 - keep_len):
        posisitonal_embedding_new[4 * i + keep_len] = positional_embedding_pre[
            i + keep_len
        ]
...
bigG_model, _, bigG_preprocess = open_clip.create_model_and_transforms(
    "ViT-bigG-14", pretrained="Yourpath/open_clip_pytorch_model.bin"
)
...
with torch.no_grad():
    ...
    def encode_prompt(
        ...
        if lora_scale is not None and isinstance(
            pipe, StableDiffusionXLLoraLoaderMixin
        ):
        ...
        if prompt_embeds is None:
            ...
            for prompt, tokenizer, text_encoder in zip(
                ...
                if untruncated_ids.shape[-1] >= text_input_ids.shape[
                    ...
                    removed_text = tokenizer.decoder(
        ...
        if (
            ...
            for negative_prompt, tokenizer, text_encoder in zip(
                ...
                uncond_input = tokenizer(negative_prompt)

                negative_prompt_embeds = text_encoder(
                    uncond_input.to(device),
                )
                # We are only ALWAYS interested in the pooled output of the final text encoder
                negative_pooled_prompt_embeds = bigG_model.encode_text(
                    uncond_input.to(device)
                )
